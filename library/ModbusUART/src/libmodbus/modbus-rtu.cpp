...
static ssize_t _modbus_rtu_send(modbus_t *ctx, uint8_t *req, int req_length)
{
#if defined(_WIN32)
    ...
    size = ctx_rtu->rs485->write(req, (uint8_t)req_length);
    //ctx_rtu->rs485->read();
...
#ifdef ARDUINO
modbus_t* modbus_new_rtu(RS485 *rs485, unsigned long baud, uint16_t config)
#else
